...
using NConfig.Impl.Translators.Collection;

namespace NConfig.Impl.Translators
{
    public class OpenGenericDecoratorProvider : IValueTranslatorProvider
    {
        public OpenGenericDecoratorProvider(IValueTranslatorProvider inner)
        {
            this.Inner = inner;
            ...
            { 
                { typeof(IList<>), typeof(ListTranslator<>) },
                { typeof(IDictionary<,>), typeof(DictionaryTranslator<,>) },
                { typeof(IEnumerable<>), typeof(EnumerableTranslator<>) },
                { typeof(ICollection<>), typeof(CollectionTranslator<>) },
            };
        }
        ...
        private IValueTranslator BuildTranslatorForGenericType(Type type)
        {
            ...
            var argumentsTranslatorTypes = genericArguments.Select(arg => typeof(IValueTranslator<>).MakeGenericType(arg)).ToArray();
